"""
verify_china_urls.py - Test China.list candidate URLs for accessibility.

For each URL, performs a quick HEAD request (falling back to GET) to check
if the resource is accessible, then infers the appropriate parser type.

Usage:
    python scripts/verify_china_urls.py

Output:
    Prints status for each URL. At the end, prints a summary of accessible
    URLs with inferred parser types, formatted as Python tuples ready to
    paste into GenerateAll.py.
"""
...
# Add this directory to path so we can import common
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

# Default headers matching common.py
DEFAULT_HEADERS = {
    "User-Agent": (
...
def infer_parser_type(url):
    """Infer the parser type from the URL pattern and file extension."""
    url_lower = url.lower()

    # Skip m3u8/m3u playlists (TV streams, not rules)
    if url_lower.endswith(".m3u8") or url_lower.endswith(".m3u"):
        return None  # Skip these

    # Ruleset.isagood.day uses Surge-format .conf files
    if "ruleset.isagood.day" in url_lower:
        return "surge"

    # Explicit file extension mapping
    if url_lower.endswith(".yaml"):
        return "yaml"

    if url_lower.endswith(".module"):
        return "sgmodule"

    # .conf files from Surge
    if url_lower.endswith(".conf"):
        return "surge"
    ...
    # .txt files - need repo-specific logic
    if url_lower.endswith(".txt"):
        # gaoyifan/china-operator-ip - plain CIDR lists
        if "gaoyifan/china-operator-ip" in url_lower:
            return "plaincidr"
        # misakaio/chnroutes2 - plain CIDR
        if "misakaio/chnroutes2" in url_lower:
            return "plaincidr"
        # Loyalsoldier rules use loyalsoldier parser
        if "loyalsoldier" in url_lower:
            return "loyalsoldier"
        # Hackl0us/GeoIP2-CN - plain CIDR
        if "hackl0us/geoip2-cn" in url_lower or "hackl0us/geoip2" in url_lower:
            return "plaincidr"
        # Default: try loyalsoldier parser (handles rulesets)
        return "loyalsoldier"
...
def check_url(url):
    """Check if a URL is accessible. Returns (url, accessible, parser_type, status_code)."""
    parser_type = infer_parser_type(url)
    if parser_type is None:
        return (url, False, None, "SKIPPED")

    try:
        # First try HEAD request
        r = requests.head(url, headers=DEFAULT_HEADERS, timeout=30, allow_redirects=True)
        if r.status_code == 200:
            return (url, True, parser_type, 200)
        # Fallback: try GET
        r = requests.get(url, headers=DEFAULT_HEADERS, timeout=60, stream=True)
        if r.status_code == 200:
            return (url, True, parser_type, 200)
        return (url, False, parser_type, r.status_code)
    except requests.exceptions.Timeout:
        return (url, False, parser_type, "TIMEOUT")
    except requests.exceptions.ConnectionError:
        return (url, False, parser_type, "CONN_ERR")
    except Exception as e:
        return (url, False, parser_type, str(e))


def main():
    print("=" * 70)
    print("Verifying {} candidate URLs for China.list expansion...".format(len(CANDIDATE_URLS)))
    print("=" * 70)
    print()
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        ...
        for future in as_completed(future_to_url):
            ...
            result = check_url(url)  # Re-run for simplicity
            # Actually, let's collect from future
            try:
                result = future.result()
            except Exception as e:
                result = (url, False, None, str(e))

            url, ok, ptype, status = result
            completed += 1
            filename = url.rstrip("/").split("/")[-1]

            if ok:
                accessible.append((url, ptype))
                status_str = "OK"
            else:
                inaccessible.append((url, status))
                status_str = "FAIL"

            print("  [{:4}/{:4}] {:15s} | {} | {}".format(
                completed, total, status_str, str(status), filename
            ))

    print()
    print("=" * 70)
    print("RESULTS: {} accessible, {} inaccessible".format(
        len(accessible), len(inaccessible)))
    print("=" * 70)
    print()

    if accessible:
        print("--- ACCESSIBLE URLs (ready for GenerateAll.py) ---")
        print()
        for url, ptype in accessible:
            print("        (\"{}\", \"{}\", None),".format(url, ptype))
        print()

    if inaccessible:
        print("--- INACCESSIBLE URLs ---")
        for url, reason in inaccessible:
